...
4. **Never self-modify steering files.** `CLAUDE.md` (global or project) and `.claude/` (settings, rules, skills, MCP config) persist across every future session and are an injection surface. Changes to these files require the operator's explicit approval, reviewed with the same scrutiny as CI/CD pipeline changes.
...
- `.gitignore` does **not** stop the agent from reading files — it reads the filesystem directly. Do not open, cat, or include `.env`, `*.pem`, `*.key`, `credentials.json`, or SSH keys in the conversation context; once read, their contents are sent to the model provider.
